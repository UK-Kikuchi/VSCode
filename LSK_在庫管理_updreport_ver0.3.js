        ...
        myIndexButton.onclick = async () => {
            ...
            if (pass.value == '12345678') {
                ...
                if (result) {
                    ...
                    //console.log(ins_record);
                    for (var j = 0; j < ins_record.length; j++) {
                        //console.log("*** 商品プライマリ ***");
                        //console.log(ins_record[j].item.value);
                        // 商品を検索
                        var params = {
                            "app": 40, // 商品マスタ 
                            'query': '文字列__1行_ = "' + ins_record[j].item.value + '"',
                            'fields': ['レコード番号', 'チェックボックス_1', '仕入原料']
                        };
                        //console.log(params.query);

                        const resp2 = await kintone.api(kintone.api.url('/k/v1/records', true), 'GET', params);
                        //console.log("*** 商品情報 ***");
                        //console.log(resp2.records[0].チェックボックス_1.value);
                        //console.log(resp2.records[0].仕入原料.value);
                        // 原料情報がる場合のみデータ作成

                        for (var k = 0; k < resp2.records[0].仕入原料.value.length; k++) {

                            var p_genryo = resp2.records[0].仕入原料.value[k].value.ルックアップ_0.value;
                            var p_haigyo = resp2.records[0].仕入原料.value[k].value.配合率.value;
                            // 仕入原料のテーブルのデータの数だけレコードを作成する
                            var record = {
                                "受注情報": { "value": ins_record[j].order_id.value },
                                "商品情報": { "value": ins_record[j].item.value },
                                "単位": { "value": ins_record[j].unit.value },
                                "数量": { "value": ins_record[j].qty.value },
                                "容量": { "value": ins_record[j].yoryo.value },
                                "原料情報": { "value": p_genryo },
                                "配合比率": { "value": p_haigyo },
                            }
                            param_value_arry.push(record);
                            console.log("*** 原料情報 *** ");
                            console.log(ins_record[j].unit.value);
                            console.log(ins_record[j].qty.value);
                            console.log(ins_record[j].yoryo.value);
                            console.log(p_genryo);
                            console.log(p_haigyo);
                        }
                    }
